...
class Vehicle(mesa.Agent):
    ...
    def __init__(self, unique_id, pos, direction, percentagepesero, model):
        ...
        # Set if it is an pesero vehicle
        maxpesero = 100 * percentagepesero
        pesero = random.randint(0, 200)
        if pesero <= maxpesero:
            self.pesero = True
        else:
            self.pesero = False
    ...
    def move(self):
        ...
        cellmates1 = self.model.grid.get_cell_list_contents(new_pos1)
        cellmates2 = self.model.grid.get_cell_list_contents(new_pos2)
        cellmates3 = self.model.grid.get_cell_list_contents(new_pos3)
        cellmates4 = self.model.grid.get_cell_list_contents(new_pos4)
